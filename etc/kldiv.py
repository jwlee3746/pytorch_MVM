...
if __name__ == '__main__':
    ...
    kld_ = nn.KLDivLoss(reduction="batchmean")
    jsd_ = JSD()
...
###---------------------------------------- Train ----------------------------------------
    for epoch in range(1, opt.ep + 1):
        ...
        for idx, target in enumerate(tqdm(train_loader, leave=True, desc='[%d/%d]' % (epoch, opt.ep))) :
            ...
            #pd_rf = pairwise_distances(ml_real_out, ml_fake_out)
            #zeros = torch.zeros(pd_rf.shape).to(DEVICE) 
                    
            p_dist =  torch.dist(pd_r,pd_f,2)         
            c_dist = torch.dist(ml_real_out.mean(0),ml_fake_out.mean(0),2)         
            #r_dist =  torch.dist(pd_rf,zeros,2)     
            
            #pd_kl = kld_(pd_r, pd_f)
            pd_kl = jsd_(pd_r, pd_f)
            #############################
...
###--------------------write_logs----------------------------------------              
        ...
        print(' pd_kl:', pd_kl.item(),' c_dist:',c_dist.item(), ' p_dist:', p_dist.item(),' triplet_loss:',triplet_loss.item())
