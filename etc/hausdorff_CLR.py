...
from loss import TripletLoss, pairwise_distances, AveragedHausdorffLoss, metricLoss
from model import Generator28_linear, Generator64, Generator128, ML28, ML64, ML128, MappingNetwork, gen64_DCGAN, ML64_DCGAN
from ImageDataLoader import ContrastiveLearningDataset
...
parser.add_argument('--n_views', default=2, type=int, help='n_views')
...
parser.add_argument('--g_lr', default=2e-2, type=float, help='train learning rate')
...
#parser.add_argument('--tau', default=0.2, type=float, help='cross-entropy temperature')
#parser.add_argument('--hyp_c', default=0.1, type=float, help='hyperbolic c, "0" enables sphere mode')
parser.add_argument('--delta', default=1, type=float, help='triplet loss density guidance weight parameter')
...
if __name__ == '__main__':
    ...
    if opt.data_name == 'MNIST':
        ...
        dataset = ContrastiveLearningDataset(opt.data_path)
        trainset = dataset.get_dataset(opt.data_name, opt.size, opt.n_views)
    ...
    for epoch in range(1, opt.ep + 1):
        ...
        for idx, target in enumerate(tqdm(train_loader, leave=True, desc='[%d/%d]' % (epoch, opt.ep))) :
            ...
            if type(images) == list:
                aug_images = images[1]
                images = images[0]
                aug_images = Variable(aug_images).to(DEVICE)
            ...
            t_loss = triplet_(ml_real_out, ml_aug_out, ml_fake_out) #- torch.dist(ml_real_out.mean(0),ml_fake_out.mean(0),2) 
...
###--------------------write_logs----------------------------------------              
            ...
            if opt.write_logs:
                ...
                writer.add_scalar("t_loss/train", t_loss, epoch)
