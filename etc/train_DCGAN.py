...
parser.add_argument('--out_dim', default=1, type=int, help='ML network output dim')
...
parser.add_argument('--ml_lr', default=2e-4, type=float, help='train learning rate')
...
###---------------------------------------- Train ----------------------------------------
    for epoch in range(1, opt.ep + 1):
        ...
        for target in tqdm(train_loader, leave=True, desc='[%d/%d]' % (epoch, opt.ep)) :
            ...
            ml_real_out = netML(real_img).view(batch_size, -1)
            D_real_loss = BCE_loss(ml_real_out.squeeze(), y_real_)

            z_ = torch.randn((batch_size, 128)).view(-1, 128, 1, 1)
            z_ = Variable(z_.cuda())
            fake_img = netG(z_)

            ml_fake_out = netML(fake_img).view(batch_size, -1)
            D_fake_loss = BCE_loss(ml_fake_out.squeeze(), y_fake_)
            ...
            G_train_loss = BCE_loss(ml_fake_out.squeeze(), y_real_)
...
###--------------------Hausdorff distance----------------------------------------
        ...
        if opt.write_logs:
            writer.add_scalars("loss/train", {"D_train_loss": D_train_loss, "G_train_loss": G_train_loss, \
                                              "triplet_loss": triplet_loss, "p_dist": p_dist, "c_dist": c_dist,\
        ...
        print(' D_real_loss:',D_real_loss.item(), ' D_fake_loss:', D_fake_loss.item(),' G_train_loss:',G_train_loss.item())
