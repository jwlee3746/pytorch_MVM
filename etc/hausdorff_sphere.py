...
from loss import TripletLoss, pairwise_distances, AveragedHausdorffLoss, metricLoss, CosineAveragedHausdorffLoss
...
parser.add_argument('--g_lr', default=2e-2, type=float, help='train learning rate') # 2e-4
...
parser.add_argument('--norm_momentum', default=1.0, type=float, help='norm_momentum')
parser.add_argument('--sec_wei', default=1.0, type=float, help='sec_wei')
parser.add_argument('--l2reg_wei', default=0.0, type=float, help='l2reg_wei') # 1e-4
...
if __name__ == '__main__':
    ...
    copy('train_MNIST.py', code_path+'/train_MNIST.py')
    ...
    c_hausdorff_ = CosineAveragedHausdorffLoss(directed = True).to(DEVICE)
    #metric_ = metricLoss(opt.margin, opt.alpha, opt.delta).to(DEVICE)
...
###---------------------------------------- Train ----------------------------------------
    for epoch in range(1, opt.ep + 1):
        ...
        for idx, target in enumerate(tqdm(train_loader, leave=True, desc='[%d/%d]' % (epoch, opt.ep))) :
            ...
            #####
            fea_norm = ml_real_out.norm(p=2, dim=1)
                        
            sp_loss = F.relu(100 - fea_norm.mean())
            ...
            if idx == 0:
                record_norm_mean = norm_mean.detach()
            else:
                record_norm_mean = (1 - opt.norm_momentum) * record_norm_mean + \
                    opt.norm_momentum * norm_mean.detach()
            
            #print(record_norm_mean)
            loss_sec = ((fea_norm - record_norm_mean) ** 2).mean()
            #print(loss_sec)
            loss_l2reg = (fea_norm ** 2).mean()
            #print(loss_l2reg)
            
            norm_temp = ml_real_out.norm(dim=1, p=2, keepdim=True)
            if True:
                ml_real_out = ml_real_out.div(norm_temp.expand_as(ml_real_out))
            '''
            '''
            ...
            ml_loss = metric_(ml_real_out,ml_real_out_shuffle,ml_fake_out)
            #ml_loss = triplet_(ml_real_out,ml_real_out_shuffle,ml_fake_out)
            '''
            if opt.sec_wei != 0:
                ml_loss = ml_loss + opt.sec_wei * loss_sec
            if opt.l2reg_wei != 0:
                ml_loss = ml_loss + opt.l2reg_wei * loss_l2reg
            '''
            netML.zero_grad()
            ml_loss.backward()
            optimizerML.step() 
            ...
            #g_loss = c_hausdorff_(ml_real_out, ml_fake_out)
            ...
            with torch.no_grad(): 
                ...
                noise_embedding = torch.cat([noise_embedding, z.squeeze()], dim=0)
            ### For recording manifold ###
