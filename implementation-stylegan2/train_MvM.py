...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if real_img_aug.size()[0] != args.batch:
            continue            
        ...
        d_loss = triplet_(real_pred,real_pred_shuffle,fake_pred_shuffle)         
...
#                 real_img_aug = real_img                
            ...
            discriminator.zero_grad()
            (args.r1 / 2 * r1_loss * args.d_reg_every + 0 * real_pred[0].mean()).backward()
        ...
        if get_rank() == 0:
            ...
            if i % 300 == 0:
                with torch.no_grad():
                    ...
                    utils.save_image(
                        sample,
                        f"{log_folder}/sample/{str(i).zfill(6)}.png",
            ...
            if i % 10000 == 0:
                torch.save(
                    ...
                    f"{log_folder}/checkpoint/{str(i).zfill(6)}.pt",
                )
...
#     ).to(device) 
    ...
    d_optim = optim.Adam(
        ...
        betas=(0.5 ** d_reg_ratio, 0.999 ** d_reg_ratio),
    )
